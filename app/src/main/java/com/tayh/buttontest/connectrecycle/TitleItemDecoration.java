...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, List<RightBean> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
...
//
//                } else {//其他的通过判断
//                    if (0 != mDatas.get(position).getType() && mDatas.get(position).getType()!=(mDatas.get(position - 1).getType())) {
...
//        mPaint.getTextBounds("5555", 0, 100, mBounds);
//        c.drawText("2222", child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }

    @Override
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        int pos = ((LinearLayoutManager)(parent.getLayoutManager())).findFirstVisibleItemPosition();
        Log.i("zeyu","onDrawOver");
        int tag = mDatas.get(pos).getType();
        ...
        mPaint.setColor(COLOR_TITLE_FONT);
        mPaint.getTextBounds("1111", 0, 4, mBounds);
        c.drawText("", child.getPaddingLeft(),
